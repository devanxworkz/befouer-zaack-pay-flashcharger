...
const getDisplaySOC = (actualSOC) => {
  const minActual = 16;
  const cutoff = 90;

  if (actualSOC <= minActual) {
    return 0;
  }

  if (actualSOC < cutoff) {
    return ((actualSOC - minActual) * 90) / (cutoff - minActual);
  }
...
export default function ChargingPreferences() {
  ...
  const actualSOC = vehicleData?.soc || 0;
const displaySOC = getDisplaySOC(actualSOC);
...
useEffect(() => {
  ...
  setTarget(Math.ceil(displaySOC));
}
  }, [vehicleData]);
  ...
  const currentAh = (displaySOC / 100) * fullAh;
  ...
  console.log("actualSOC:", actualSOC);
console.log("displaySOC:", displaySOC);
console.log("target:", target);
console.log("currentAh:", currentAh);
console.log("requiredAh:", requiredAh);
console.log("requiredKwhFinal:", requiredKwhFinal);
...
const options = {
 ...
 handler: async function (response) {
  ...
  ondismiss: function () {
    ...
    setPaymentLoading(false); // ✅ ENABLE BUTTON AGAIN
  }
}
    };
  ...
  setPaymentLoading(false); // ✅ IMPORTANT
